...
SELECT p.nome AS Paciente, m.nome AS Medico, c.dataconsulta, c.horaconsulta, a.andar
	FROM Pacientes p, Medicos m, Consulta c, Ambulatorio a
    WHERE p.codp = c.codp AND
		m.codm = c.codm AND
        m.nroa = a.nroa;
        
SELECT f.nome AS Nome,(f.salario / (COUNT(d.codd)+1)) AS 'Renda Per-capita' 
		FROM Funcionarios f INNER JOIN Dependetes d
			ON f.codf = d.codf
		GROUP BY d.codf;
        
Select nome, (salario-(salario * 0.1)) as líquido
From Funcionarios

Select count(numero) as Total
From Medicos;

Select count(*) as TotalOrtopedistas From
Medicos
Where especialidade = 'ortopedista';

Select * from Medicos
where cidade = especialidade;

Select nome from Médicos m
Where not exists
...
ALTER TABLE Medicos
ADD FOREIGN KEY(numero) REFERENCES Ambulatorios(numero);
ALTER TABLE Funcionarios
ADD FOREIGN KEY(numero) REFERENCES Ambulatorios(numero);

select rg, nome from Pacientes
where nome like

Select nome from medicos,consultas
Where medico.CRM=consultas.CRM and
data= '2008-09-29' and
hora between '08:00' and '12:00'
Ou
Select nome from Medicos
Where exists (Select * from consultas
Where data = '2008-09-29' and
CRM= Medicos.CRM
and hora < '12:00')
...
select * from funcionarios f left join dependetes d
on f.codf=d.codf
where d.codd is null;


select * from funcionarios f right join dependetes d
on f.codf=d.codf
...
alter table dependetes
drop foreign key dependetes_ibfk_1;

delete from funcionarios where codf = 7;


select * from ambulatorios a left join medicos m on m.nroa = a.nroa
where m.codm is null;



select p.nome as paciente, m.nome as medico, c.data, c.hora, a.andar from consultas c 
join pacientes p on c.codp = p.codp
join medicos m on m.codm = c.codm
join ambulatorios a on a.nroa = m.nroa;


select f.codf, f.nome,round(salario/(1+count(*)),2) as renda_per_capita from funcionarios f 
join dependetes d on f.codf = d.codf group by f.codf,f.nome,f.salario;

select * from  funcionarios f 
join dependetes d on f.codf = d.codf;


select m.nome as medico,p.nome as paciente, count(*) as consultas 
from consultas c inner join medicos  m
on c.codm = m.codm
inner join pacientes p
on p.codp=c.codp
group by medico, paciente
order by consultas desc limit 9;
